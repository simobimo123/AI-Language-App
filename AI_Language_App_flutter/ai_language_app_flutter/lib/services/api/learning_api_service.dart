...
class LearningApiService {
  ...
  Future<List<dynamic>> getLearningProfiles() async {
    final response = await client.get('/learning/profiles', authenticated: true);
    final data = client.decodeResponse(response);
    if (response.statusCode == 200) return List<dynamic>.from(data as List);
    throw client.apiException(data, 'Failed to get learning profiles', statusCode: response.statusCode);
  }

  Future<Map<String, dynamic>> getCurrentLearningProfile() async {
    final response = await client.get('/learning/current', authenticated: true);
    final data = client.decodeResponse(response);
    if (response.statusCode == 200) return Map<String, dynamic>.from(data as Map);
    throw client.apiException(data, 'Failed to get current learning profile', statusCode: response.statusCode);
  }
  ...
  Future<Map<String, dynamic>> switchLearningLanguage({required String language}) async {
    final response = await client.put('/learning/current/$language', authenticated: true);
    final data = client.decodeResponse(response);
    if (response.statusCode == 200) return Map<String, dynamic>.from(data as Map);
    throw client.apiException(data, 'Failed to switch learning language', statusCode: response.statusCode);
  }

  Future<Map<String, dynamic>> getLearningPath() async {
    final response = await client.get('/learning/path', authenticated: true);
    final data = client.decodeResponse(response);
    if (response.statusCode == 200) return Map<String, dynamic>.from(data as Map);
    throw client.apiException(data, 'Failed to get learning path', statusCode: response.statusCode);
  }

  Future<Map<String, dynamic>> getLessonContent({required int lessonId}) async {
    final response = await client.get('/lesson-content/$lessonId', authenticated: true);
    final data = client.decodeResponse(response);
    if (response.statusCode == 200) return Map<String, dynamic>.from(data as Map);
  ...
  Future<Map<String, dynamic>> submitLessonAssessment({
    ...
    final response = await client.post(
      ...
      body: jsonEncode({'conversation_id': conversationId, 'answers': answers}),
    );
    final data = client.decodeResponse(response);
    if (response.statusCode == 200) return Map<String, dynamic>.from(data as Map);
    throw client.apiException(data, 'Failed to submit lesson assessment', statusCode: response.statusCode);
  }
