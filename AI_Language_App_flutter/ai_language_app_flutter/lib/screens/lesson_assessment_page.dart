...
class _LessonAssessmentPageState extends State<LessonAssessmentPage> {
  ...
  String _title() => _text(ar: 'اختبار الدرس', en: 'Lesson assessment', fr: 'Évaluation de la leçon', es: 'Evaluación de la lección', de: 'Lektionstest', it: 'Verifica della lezione', ja: 'レッスンテスト', ko: '레슨 평가', zh: '课程测试');
  String _loadingText() => _text(ar: 'جاري تحميل الاختبار...', en: 'Loading assessment...', fr: 'Chargement...', es: 'Cargando...', de: 'Test wird geladen...', it: 'Caricamento...', ja: '読み込み中...', ko: '불러오는 중...', zh: '正在加载...');
  String _nextLabel() => _text(ar: 'التالي', en: 'Next', fr: 'Suivant', es: 'Siguiente', de: 'Weiter', it: 'Avanti', ja: '次へ', ko: '다음', zh: '下一题');
  String _submitLabel() => _text(ar: 'إرسال الإجابات', en: 'Submit answers', fr: 'Envoyer les réponses', es: 'Enviar respuestas', de: 'Antworten abgeben', it: 'Invia risposte', ja: '回答を送信', ko: '답변 제출', zh: '提交答案');
  String _retryLabel() => _text(ar: 'إعادة المحاولة', en: 'Try again', fr: 'Réessayer', es: 'Intentar de nuevo', de: 'Erneut versuchen', it: 'Riprova', ja: 'もう一度', ko: '다시 시도', zh: '重新尝试');
  String _continueLabel() => _text(ar: 'متابعة', en: 'Continue', fr: 'Continuer', es: 'Continuar', de: 'Weiter', it: 'Continua', ja: '続ける', ko: '계속', zh: '继续');
  String _scoreLabel() => _text(ar: 'النتيجة', en: 'Score', fr: 'Score', es: 'Puntuación', de: 'Ergebnis', it: 'Punteggio', ja: 'スコア', ko: '점수', zh: '得分');
  String _passingLabel() => _text(ar: 'درجة النجاح', en: 'Passing score', fr: 'Score requis', es: 'Puntuación mínima', de: 'Bestehensgrenze', it: 'Punteggio minimo', ja: '合格点', ko: '통과 점수', zh: '及格分数');
  String _bestLabel() => _text(ar: 'أفضل نتيجة', en: 'Best score', fr: 'Meilleur score', es: 'Mejor puntuación', de: 'Beste Punktzahl', it: 'Miglior punteggio', ja: 'ベストスコア', ko: '최고 점수', zh: '最佳得分');
  ...
  double _toDouble(dynamic value, double fallback) => value is num ? value.toDouble() : double.tryParse(value?.toString() ?? '') ?? fallback;

  Future<void> _submit() async {
    if (_submitting || _questions.isEmpty || _answers.length != _questions.length) return;
    setState(() { _submitting = true; _error = null; });
    try {
      final result = await _repository.submitLessonAssessment(
        lessonId: widget.lesson.id,
        conversationId: widget.conversationId,
        answers: _answers.entries.map((e) => {'question_id': e.key, 'answer': e.value}).toList(),
      );
      if (!mounted) return;
      await _showResult(
  ...
  Future<void> _showResult({required bool passed, required double score, required double bestScore, required bool levelUpgraded, required String newLevel}) async {
  ...
  Widget build(BuildContext context) {
    ...
    if (_loading) return Scaffold(appBar: AppBar(title: Text(_title())), body: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [const CircularProgressIndicator(), const SizedBox(height: 16), Text(_loadingText())])));
    if (_error != null || _questions.isEmpty) return Scaffold(appBar: AppBar(title: Text(_title())), body: Center(child: Padding(padding: const EdgeInsets.all(24), child: Text(_error ?? 'No assessment questions available.', textAlign: TextAlign.center, style: TextStyle(color: theme.colorScheme.error)))));
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        ...
        Expanded(child: ListView(padding: const EdgeInsets.all(20), children: [
          Text(text, textDirection: _textDirection(text), style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700, height: 1.35)),
          const SizedBox(height: 24),
          ...options.map((option) {
            final optionId = option['id']?.toString() ?? '';
            final optionText = option['text']?.toString() ?? '';
            return Padding(padding: const EdgeInsets.only(bottom: 12), child: RadioListTile<String>(
              value: optionId,
              groupValue: selected,
              onChanged: _submitting ? null : (value) { if (value != null) setState(() => _answers[id] = value); },
              title: Text(optionText, textDirection: _textDirection(optionText)),
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16), side: BorderSide(color: selected == optionId ? theme.colorScheme.primary : theme.colorScheme.outlineVariant)),
            ));
          }),
        ])),
  ...
  TextDirection _textDirection(String text) => RegExp(r'[\u0600-\u06FF]').hasMatch(text) ? TextDirection.rtl : TextDirection.ltr;
}
