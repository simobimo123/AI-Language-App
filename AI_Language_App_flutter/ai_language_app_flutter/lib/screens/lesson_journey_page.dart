...
class _LessonJourneyPageState extends State<LessonJourneyPage> {
  ...
  Future<void> _loadStages() async {
    ...
    try {
      ...
      final raw = data['stages'];
      final stages = raw is Map ? Map<String, dynamic>.from(raw) : <String, dynamic>{};
      String readStatus(String key, String fallback) {
        final value = stages[key];
        if (value is String && value.isNotEmpty) return value;
        if (value is Map && value['status'] != null) return value['status'].toString();
        return fallback;
      }
      if (!mounted) return;
      setState(() {
      ...
      if (!mounted) return;
      setState(() { _loading = false; _error = _t(ar: 'تعذر تحميل حالة مراحل الدرس.', en: 'Could not load the lesson stage status.'); });
    }
  }
  ...
  Future<void> _openLearn() async {
    final completed = await Navigator.of(context).push<bool>(MaterialPageRoute(builder: (_) => LessonLearnPage(lesson: widget.lesson, languageController: widget.languageController)));
    if (!mounted || completed != true) return;
    try { await _saveStage('learn'); } catch (_) { if (mounted) _showSaveError(); }
  }

  Future<void> _openTeaching() async {
    if (!_learnCompleted) return;
    final completed = await Navigator.of(context).push<bool>(MaterialPageRoute(builder: (_) => LessonTeachingPage(lesson: widget.lesson, languageController: widget.languageController)));
    if (!mounted || completed != true) return;
    try { await _saveStage('teaching'); } catch (_) { if (mounted) _showSaveError(); }
  }

  Future<void> _openPractice() async {
    if (!_teachingCompleted) return;
    final completed = await Navigator.of(context).push<bool>(MaterialPageRoute(builder: (_) => LessonPracticePage(lesson: widget.lesson, languageController: widget.languageController)));
    if (!mounted || completed != true) return;
    try {
    ...
    } catch (_) { if (mounted) _showSaveError(); }
  }

  void _showSaveError() {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(_t(ar: 'تعذر حفظ حالة المرحلة على الخادم.', en: 'Could not save the stage status on the server.'))));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: DecoratedBox(
        decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [theme.colorScheme.primary.withValues(alpha: .10), theme.scaffoldBackgroundColor])),
        child: SafeArea(
          child: _loading
              ? const Center(child: CircularProgressIndicator())
              : CustomScrollView(slivers: [
                  SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.fromLTRB(12, 8, 20, 8), child: Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      const SizedBox(height: 3), Text(widget.lesson.title, style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900)),
                    ])),
                  ...
                  SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.fromLTRB(20, 10, 20, 24), child: Container(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      const SizedBox(height: 16), Text(_t(ar: 'أكمل المراحل بالترتيب. حالة كل مرحلة محفوظة على الخادم، لذلك تبقى معك عند إعادة فتح الدرس.', en: 'Complete the stages in order. Each stage is saved on the server and remains available when you reopen the lesson.'), style: const TextStyle(color: Colors.white70, height: 1.45)),
                      const SizedBox(height: 18), ClipRRect(borderRadius: BorderRadius.circular(99), child: LinearProgressIndicator(value: progress, minHeight: 9, backgroundColor: Colors.white.withValues(alpha: .18), valueColor: const AlwaysStoppedAnimation(Colors.white))),
                      const SizedBox(height: 8), Text('${(progress * 100).round()}%', style: const TextStyle(color: Colors.white70, fontWeight: FontWeight.w800)),
                    ]),
                  ))),
                  SliverPadding(padding: const EdgeInsets.fromLTRB(20, 0, 20, 34), sliver: SliverList.list(children: [
