...
class PlacementTestView extends StatelessWidget {
  ...
  const PlacementTestView({super.key, required this.controller, required this.themeController, required this.languageController});

  bool get isArabic => languageController.locale.languageCode == 'ar';
  String text({required String ar, required String en}) => isArabic ? ar : en;
  ...
  Widget build(BuildContext context) {
    ...
    if (controller.isFinished) return _FinishedView(level: controller.finalLevel ?? controller.currentLevel, text: text);
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        _LevelCard(icon: Icons.school_rounded, label: text(ar: 'المستوى الحالي: ${levelName(controller.currentLevel)}', en: 'Current level: ${levelName(controller.currentLevel)}'), theme: theme),
        const SizedBox(height: 18),
        Wrap(
          spacing: 10,
          runSpacing: 10,
        ...
        if (controller.errorMessage != null) ...[
          ...
          _ErrorCard(message: controller.errorMessage!, onRetry: controller.retry),
        ],
        const SizedBox(height: 20),
        SizedBox(
          height: 54,
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 18),
        _LevelCard(icon: Icons.quiz_rounded, label: text(ar: 'مستوى الاختبار: ${levelName(controller.currentLevel)}', en: 'Test level: ${levelName(controller.currentLevel)}'), theme: theme),
        const SizedBox(height: 20),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  RadioGroup<int>(
                    groupValue: selected,
                    onChanged: controller.isEvaluating ? null : (value) {
        ...
        if (controller.errorMessage != null) _ErrorCard(message: controller.errorMessage!, onRetry: controller.retry),
        const SizedBox(height: 8),
        SizedBox(
          height: 54,
          child: FilledButton(
