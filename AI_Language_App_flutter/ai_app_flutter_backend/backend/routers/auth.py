...
if not GOOGLE_WEB_CLIENT_ID:
    raise RuntimeError("GOOGLE_WEB_CLIENT_ID must be set in the environment")
...
def get_current_user(
    ...
    explanation_mode = user.tutor_explanation_language_mode
...
def google_login(user_data: GoogleLogin, db: Session = Depends(get_db)):
    ...
    if user is None:
        ...
        user = User(
            ...
            tutor_explanation_language_mode="native",
            is_active=True,
        )
        db.add(user)
        db.commit()
