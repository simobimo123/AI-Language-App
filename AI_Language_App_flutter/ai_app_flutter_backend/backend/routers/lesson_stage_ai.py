...
def _history_messages(
    ...
    for item in history[-max_messages:]:
        ...
        content = _remove_control_markers(
...
def _current_target_order(
    ...
    required_orders = [
        int(t["order"])
        for t in targets
        if t["required"] and t.get("order") is not None
    ]
...
def _all_required_targets_completed(
    ...
    return (
        bool(required_orders)
        and required_orders.issubset(completed_orders)
    )
...
def _teaching_system_prompt(
    *,
    ...
    if current_target is None:
        current_target_text = (
            "**CURRENT TARGET**: There is no remaining target."
        )
    else:
        goal = str(
        ...
        patterns = [
            str(x).strip()
            for x in current_target.get("patterns", [])
            if str(x).strip()
        ]
    ...
    if current_target_order is not None:
        for target in targets:
            if (
                target["required"]
                and target.get("order") is not None
                and int(target["order"])
                > int(current_target_order)
            ):
                next_target = target
                break
    ...
    if next_target is None:
        next_target_text = (
            "**NEXT TARGET**: None; this is the final required target."
        )
    else:
        next_target_text = (
            f"**NEXT TARGET**: "
            f"{next_target.get('order')} — "
            f"{str(next_target.get('goal', '')).strip()}"
        )

        patterns = [
            str(x).strip()
            for x in next_target.get("patterns", [])
            if str(x).strip()
        ]

        if patterns:
            next_target_text += (
                f"\n**NEXT TARGET PATTERNS**: "
                f"{' | '.join(patterns)}"
            )

    if is_start:
        task = (
            "**START**: This is the first turn. "
            "Teach the current target, give a clear model, "
            "then ask the learner for one parallel answer."
        )
    else:
        task = (
            "**RESPONSE**: Evaluate the learner's latest answer. "
            "If it is not mastered, correct and retry the same target. "
            "If it is mastered, teach and model the next target "
...
You are a professional, patient, academically structured language teacher.
You are NOT a casual chatbot and NOT the practice conversation partner.
...
8. NEXT TARGET: After completion, teach the next target and give its model BEFORE asking the learner to answer.

**BEFORE EVERY NEW TARGET QUESTION**:

- The learner must first have been taught how to answer the target.

- The reply must contain a short explanation or meaning AND a concrete model/example before the question or prompt.
...
- The learner-facing `reply` may use these two visual markers when useful:

  - `"text"` means the text should be displayed in RED by the app. Do not include the quote characters in the visible result.

  - `*text*` means the text should be displayed in GREEN by the app. Do not include the asterisks in the visible result.

- Use these markers sparingly and only when they improve learning clarity.

- For corrections, use the red form for the learner's incorrect part and the green form for the corrected part.

- Example: `"Ich heißen" → *Ich heiße*`.

- These markers are UI formatting instructions for the learner-facing `reply` only. Do not use them for metadata or JSON field names.
...
def _practice_system_prompt(
    *,
    ...
    task = (
        "**START**: Begin naturally from the scenario, "
        "use one suitable target, and invite one response."
        if is_start
        else
        "**CONTINUE**: Respond to the learner's actual message "
        "and use it to continue naturally."
    )
...
- For a correction, use the red form for the incorrect part and the green form for the corrected part.

- Example: `"Ich heißen" → *Ich heiße*`.

- These markers are only for the learner-facing response.
    ...
    *,
    ...
    try:
        ...
        (
            ...
            marker_target,
            teaching_complete,
        ) = _extract_target_completion(raw_reply)
    ...
    if not isinstance(payload, dict):
        raise RuntimeError(
            "Teaching AI returned an invalid evaluation object."
        )
    ...
    if not reply:
        raise RuntimeError(
            "Teaching AI evaluation did not contain "
            "a learner-facing reply."
        )

    target_completed = (
        payload.get("target_completed") is True
    )

    target_order_raw = payload.get(
        "target_order"
    )
    ...
    stage_completed = (
        payload.get("stage_completed") is True
    )
    ...
    *,
    ...
    try:
        ...
        explanation_mode = str(
            user.tutor_explanation_language_mode
            or "native"
        ).strip().lower()

        if explanation_mode not in {
            "native",
            "learning",
        }:
