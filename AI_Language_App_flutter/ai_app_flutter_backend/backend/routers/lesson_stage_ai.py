...
def _prompt_context(*, lesson: CourseLesson, targets: list[dict], scenario: dict | None) -> str:
    context = (
...
def _teaching_system_prompt(*, lesson: CourseLesson, targets: list[dict], is_start: bool) -> str:
    start_rule = " First reply: speak directly to the learner in one natural message." if is_start else ""
...
def _practice_system_prompt(*, lesson: CourseLesson, targets: list[dict], scenario: dict | None, is_start: bool) -> str:
    start_rule = " First reply: one short natural message ending with one question." if is_start else ""
...
def _system_prompt(*, stage: str, lesson: CourseLesson, targets: list[dict], scenario: dict | None, is_start: bool) -> str:
    if stage == "teaching":
        return _teaching_system_prompt(lesson=lesson, targets=targets, is_start=is_start)
...
def _stream_stage_response(*, request: StageChatRequest, user_id: int, profile_id: int, lesson_id: int, conversation_id: str):
    ...
    try:
        ...
        if exchange_count_before >= MAX_STAGE_EXCHANGES and not is_control_message:
            _complete_stage(stage_progress=stage_progress, stage=request.stage, conversation_id=conversation_id)
            db.commit()
