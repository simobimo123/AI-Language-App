...
def _compact_goals(targets: list[dict]) -> str:
    ...
    return "\n".join(lines) or "- Follow objectives."

def _prompt_context(*, lesson: CourseLesson, targets: list[dict], scenario: dict | None = None) -> str:
    context = (
        f"**Lang**: {lesson.language} | **Level**: {lesson.level}\n"
        f"**Targets**:\n{_compact_goals(targets)}"
    )
    if scenario:
        context += f"\n**Scenario**: {scenario.get('title', '')}. {scenario.get('context', '')}. {scenario.get('instructions', '')}"
    return context

# ==========================================
# البرومبت المحسن والمركز للمعلم الذكي
# ==========================================
def _teaching_system_prompt(
    *,
    ...
    if current_target is None:
        current_target_text = "**STATUS**: No targets left."
    else:
        ...
        current_target_text = f"**CURR_TARGET**: {goal}"
        if patterns:
    ...
    start_rule = (
        "**TASK**: Introduce target briefly with an example, then **ask a question**."
        if is_start
        else "**TASK**: Evaluate input strictly against **SUCCESS_CRITERIA**. Address exact errors before retrying."
    )
...
**RULES**:
1. **EVALUATE**: Does input meet **SUCCESS_CRITERIA**?
2. **IF INCORRECT**: In **1 short sentence**, state the error. Give the **correct pattern** and ask to retry. **DO NOT** repeat previous feedback.
3. **IF GIBBERISH**: Say you didn't understand, remind of the pattern.
4. **IF CORRECT**: Give brief praise. **MUST** append exactly `[[TARGET_COMPLETE:{current_target_order}]]` at the very end.

**CONSTRAINTS**:
- **ANTI-LOOP**: **Never** output the same sentence twice.
- **LENGTH**: Max **2-3 short sentences**.
- **NO LABELS**: Do not print internal steps/metadata except the TARGET_COMPLETE tag.

{start_rule}
""".strip()

# ==========================================
# البرومبت المحسن والمركز لشريك المحادثة (Practice)
# ==========================================
def _practice_system_prompt(
    *,
    ...
    start_rule = (
        "**TASK**: Send **1 natural message** ending with **1 question**."
        if is_start
        else ""
    )

    return f"""
You are a **natural conversation partner**.

{_prompt_context(lesson=lesson, targets=targets, scenario=scenario)}

**RULES**:
- Use **one target at a time** naturally.
- Ask **only one question**, then **wait** for the reply.
- **NEVER** speak for the learner or invent their answers.
- Correct **major mistakes** briefly, then continue the chat. **DO NOT** make it a formal lesson.
- Keep language **short & appropriate** for the level.
- Output **only natural words**. **NO** metadata, labels, or placeholders.
...
def _stream_stage_response(*, request: StageChatRequest, user_id: int, profile_id: int, lesson_id: int, conversation_id: str):
    ...
    try:
        ...
        response = provider.generate_text(
            model=AI_MODEL,
            prompt=messages,
            system_instruction=_system_prompt(stage=request.stage, lesson=lesson, targets=targets, scenario=scenario, current_target_order=current_target_order, is_start=is_control_message),
        ...
        if request.stage == "teaching" and completed_target_order is not None:
            if current_target_order is not None and completed_target_order == current_target_order:
