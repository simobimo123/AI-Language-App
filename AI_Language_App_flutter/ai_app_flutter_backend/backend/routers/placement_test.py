...
def _get_expected_vocabulary_level(attempt: PlacementAttempt) -> str:
    """Return the only vocabulary level allowed at the current attempt stage."""
    if attempt.stage != "vocabulary":
        raise HTTPException(
            status_code=400,
            detail="This attempt is not currently at the vocabulary stage.",
        )
    ...
    if next_level is None:
        raise HTTPException(
            status_code=400,
            detail="This placement attempt has already reached the highest level.",
        )
...
def get_placement_words(
    ...
    if level == "PRE_A1":
        raise HTTPException(
            status_code=400,
            detail="PRE_A1 does not use vocabulary screening.",
        )
    ...
    if attempt_id is not None:
        ...
        if level != expected_level:
            raise HTTPException(
                status_code=400,
                detail=(
                    f"Invalid vocabulary level for this placement attempt. "
...
def evaluate_placement_words_secure(
    ...
    if attempt.stage != "vocabulary":
        raise HTTPException(
            status_code=400,
            detail="This attempt is not at the vocabulary stage.",
        )
    ...
    if len(attempt_words) != WORDS_PER_LEVEL:
        raise HTTPException(
            status_code=400,
            detail=(
                f"This attempt does not contain exactly "
                f"{WORDS_PER_LEVEL} placement words."
            ),
        )
    ...
    if invalid_selected_ids:
        raise HTTPException(
            status_code=400,
            detail=(
                "One or more selected word IDs were not part "
                "of this placement attempt."
            ),
        )
    ...
    for item in attempt_words:
        item.was_selected = (
            item.placement_vocabulary_id in selected_set
        )
    ...
    if level == "PRE_A1":
        raise HTTPException(
            status_code=500,
            detail="PRE_A1 cannot be used as a vocabulary screening level.",
        )
    ...
    if level != expected_level:
        raise HTTPException(
            status_code=500,
            detail=(
                "Placement attempt vocabulary level is inconsistent "
                f"with the expected level {expected_level}."
            ),
        )
...
def evaluate_placement_words_legacy(
    ...
    if len(presented_ids) != WORDS_PER_LEVEL:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Exactly {WORDS_PER_LEVEL} presented word IDs are required."
            ),
        )
    ...
    if len(presented_words) != WORDS_PER_LEVEL:
        raise HTTPException(
            status_code=400,
            detail=(
                "The presented word IDs do not form a valid "
                "20-word test."
            ),
        )
...
def get_placement_quiz_secure(
    ...
    if attempt.stage != "confirmation":
        raise HTTPException(
            status_code=400,
            detail=(
                "This attempt is not currently at the confirmation stage."
            ),
        )
    ...
    if attempt.preliminary_level not in LEVELS:
        raise HTTPException(
            status_code=400,
            detail="This attempt does not have a valid confirmation level.",
        )
    ...
    if len(questions) != QUIZ_QUESTIONS_PER_TEST:
        raise HTTPException(
            status_code=500,
            detail=(
                "The confirmation quiz must contain exactly "
                f"{QUIZ_QUESTIONS_PER_TEST} questions."
            ),
        )
...
def evaluate_placement_quiz_secure(
    ...
    if attempt.stage != "confirmation":
        raise HTTPException(
            status_code=400,
            detail=(
                "This attempt is not at the confirmation stage."
            ),
        )
    ...
    if len(attempt_questions) != QUIZ_QUESTIONS_PER_TEST:
        raise HTTPException(
            status_code=400,
            detail=(
                "This attempt does not contain exactly "
                f"{QUIZ_QUESTIONS_PER_TEST} quiz questions."
            ),
        )
    ...
    if len(request.answers) != QUIZ_QUESTIONS_PER_TEST:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Exactly {QUIZ_QUESTIONS_PER_TEST} answers are required."
            ),
        )
    ...
    if set(request.answers.keys()) != attempt_question_ids:
        raise HTTPException(
            status_code=400,
            detail=(
                "Answers must correspond exactly to the questions "
                "assigned to this placement attempt."
            ),
        )
    ...
    if len(question_by_id) != QUIZ_QUESTIONS_PER_TEST:
        raise HTTPException(
            status_code=500,
            detail="Placement quiz data is inconsistent.",
        )
    ...
    for attempt_question in attempt_questions:
        question = question_by_id[
            attempt_question.placement_question_id
        ]
        ...
        if selected_index < 0 or selected_index >= len(question.choices):
            raise HTTPException(
                status_code=400,
                detail=(
                    f"Invalid selected answer for question "
                    f"{question.id}."
                ),
            )
    ...
    if level not in LEVELS:
        raise HTTPException(
            status_code=400,
            detail="Invalid preliminary placement level.",
        )

    final_level = (
        level
        if passed
        else calculate_previous_level(level)
    )
...
def get_placement_quiz_legacy(
    ...
    if level == "PRE_A1":
        raise HTTPException(
            status_code=400,
            detail="PRE_A1 does not use a confirmation quiz.",
        )
...
def evaluate_placement_quiz_legacy(
    ...
    if len({answer.question_id for answer in request.answers}) != QUIZ_QUESTIONS_PER_TEST:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Exactly {QUIZ_QUESTIONS_PER_TEST} unique question IDs "
                "are required."
            ),
        )
    ...
    questions = db.execute(
        select(PlacementQuizQuestion).where(
    ...
    if len(questions) != QUIZ_QUESTIONS_PER_TEST:
        raise HTTPException(
            status_code=400,
            detail=(
                "The answer set does not correspond to exactly "
                f"{QUIZ_QUESTIONS_PER_TEST} valid questions."
            ),
        )
...
def finalize_placement_attempt(
    ...
    if attempt.status == "completed":
        if attempt.final_level is None:
            raise HTTPException(
                status_code=500,
                detail="Completed placement attempt has no final level.",
            )
    elif attempt.status != "active":
        raise HTTPException(
            status_code=400,
            detail="This placement attempt cannot be finalized.",
        )
