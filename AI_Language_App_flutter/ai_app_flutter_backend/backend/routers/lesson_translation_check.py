...
from routers.lesson_ai import _load_lesson_curriculum
...
def _get_lesson_and_profile(lesson_id: int, current_user: User, db: Session):
    ...
    if normalize_language(lesson.language) != target_language:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="This lesson does not belong to your learning language.")
    ...
    if profile is None or normalize_level(profile.level) != normalize_level(lesson.level):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="This lesson is not part of your current learning level.")

    return lesson, profile


def _translation_sentences(curriculum: dict) -> list[dict]:
    ...
    for index, item in enumerate(raw, start=1):
        if isinstance(item, str):
            ...
            continue
        if sentence:
            result.append({"id": str(index), "sentence": sentence})
...
def _learner_turn_count(user_id: int, conversation_id: str, db: Session) -> int:
    ...
    return sum(
        1 for role, content in rows
        if role == "user" and content.strip() != "START_LESSON"
    )


def _resolve_conversation(user_id: int, conversation_id: str | None, lesson_id: int, db: Session) -> str:
    if not conversation_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Continue the lesson conversation first.")
    ...
    if _learner_turn_count(user_id, candidate, db) < 2:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Continue the AI lesson before starting the translation check.")
    return candidate
...
def _evaluation_prompt(native_language: str, target_language: str, sentences: list[dict], answers: list[TranslationCheckAnswer]) -> str:
    pairs = []
...
def get_translation_check(
    ...
    lesson, _ = _get_lesson_and_profile(lesson_id, current_user, db)
    resolved = _resolve_conversation(current_user.id, conversation_id, lesson_id, db)
    curriculum = _load_lesson_curriculum(lesson)
    sentences = _translation_sentences(curriculum)
    if not sentences:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="This lesson has no translation check sentences.")
...
def submit_translation_check(
    ...
    lesson, _ = _get_lesson_and_profile(lesson_id, current_user, db)
    conversation_id = _resolve_conversation(current_user.id, data.conversation_id, lesson_id, db)
    curriculum = _load_lesson_curriculum(lesson)
    sentences = _translation_sentences(curriculum)
    if not sentences:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="This lesson has no translation check sentences.")
    ...
    if len(set(submitted_ids)) != len(submitted_ids) or set(submitted_ids) != sentence_ids:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Answer every translation question exactly once.")
    ...
    try:
        ...
        db.rollback()
        logger.exception("Translation check failed user_id=%s lesson_id=%s: %s", current_user.id, lesson_id, exc)
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Translation check is temporarily unavailable.") from exc
