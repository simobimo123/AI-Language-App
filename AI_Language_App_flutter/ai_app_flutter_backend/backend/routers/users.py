...
def update_my_profile(user_data: UserUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    new_explanation_mode = normalize_explanation_mode(user_data.tutor_explanation_language_mode)
    ...
    current_user.tutor_explanation_language_mode = new_explanation_mode
    try:
        db.commit()
    except IntegrityError:
        db.rollback()
