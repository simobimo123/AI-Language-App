...
def _get_lesson(
    ...
    current_user: User,
    db: Session,
) -> tuple[CourseLesson, LearningProfile, list[CourseLesson]]:
...
def _load_assessment(lesson: CourseLesson) -> dict:
    ...
    return {
        "passing_score": float(
            end_test.get("passing_score", lesson.passing_score)
        ),
...
def get_lesson_assessment(
    ...
    questions = _public_questions(
        assessment,
        _instruction_language(current_user),
    )
...
def submit_lesson_assessment(
    ...
    if len(answer_map) != len(data.answers):
        raise HTTPException(
            ...
            detail="Each assessment question may be answered only once.",
        )
    ...
    unknown_ids = set(answer_map) - question_ids
    if unknown_ids:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="The assessment contains an unknown question id.",
        )
    ...
    if progress is None:
        progress = get_or_create_lesson_progress(
