...
def _get_stage_progress(db: Session, user: User, profile: LearningProfile, lesson: CourseLesson) -> UserLessonStageProgress:
    progress = db.scalar(select(UserLessonStageProgress).where(UserLessonStageProgress.user_id == user.id, UserLessonStageProgress.learning_profile_id == profile.id, UserLessonStageProgress.lesson_id == lesson.id))
    if progress is None:
        progress = UserLessonStageProgress(user_id=user.id, learning_profile_id=profile.id, lesson_id=lesson.id, teaching_status="available", practice_status="locked")
        db.add(progress)
        db.flush()
...
def _practice_context(db: Session, lesson_id: int) -> dict | None:
    row = db.scalar(select(LessonPracticeScenario).where(LessonPracticeScenario.lesson_id == lesson_id).order_by(LessonPracticeScenario.scenario_order))
    if row is None:
        return None
    return {"title": str(row.title or "").strip()[:120], "context": str(row.context or "").strip()[:300], "instructions": str(row.instructions or "").strip()[:300]}
...
def _current_target_order(targets: list[dict], history) -> int | None:
    required_orders = [int(target["order"]) for target in targets if target["required"] and target.get("order") is not None]
...
def _compact_target_goals(targets: list[dict]) -> str:
    ...
    for target in targets:
        ...
        if not goal:
            continue
        prefix = f"{target.get('order')}. " if target.get("order") is not None else "- "
...
def _teaching_system_prompt(*, lesson: CourseLesson, targets: list[dict], current_target_order: int | None, is_start: bool) -> str:
    ...
    if next_target is None:
        ...
        next_patterns = [str(item).strip() for item in next_target.get("patterns", []) if str(item).strip()]
        next_target_text = f"**NEXT TARGET**: {next_target.get('order')} — {next_goal}"
        if next_patterns:
            next_target_text += f"\n**NEXT TARGET PATTERNS**: {' | '.join(next_patterns)}"
    task = "**START**: First teach/model how to produce the current target, then ask the learner to produce a parallel answer." if is_start else "**RESPONSE**: Evaluate the learner's latest answer against the target and success criteria. If not met, stay on the same target, identify meaningful errors precisely, correct/model them briefly, and ask for another attempt. If met, do NOT ask the learner to repeat the completed target; give brief positive feedback and immediately teach/model the next target before asking for it."
    return f"""
...
**CORE TEACHING METHOD — MANDATORY**:
- Follow this sequence: **TEACH → MODEL → ASK → EVALUATE → CORRECT → RETRY → NEXT TARGET**.
- **Teach before asking**: before asking the learner to produce a new target, first teach them what they need to say and how to form the answer. Do not test a target that has not been taught, except for a genuine review of something already taught.
- **Model before asking**: give one short, clear model sentence or answer pattern in the learning language, then ask the learner to produce a parallel answer of their own. Do not ask an unexplained open question when the learner has not yet been shown how to answer.
- The model must be level-appropriate and directly useful for the current target; avoid unnecessary grammar lectures.
- If the learner asks for clarification, explain briefly in the selected explanation language, then return to the current target.

**ERROR CORRECTION — MANDATORY**:
- When the learner makes a meaningful error relevant to the current target, explicitly show where the error is.
- Show the incorrect part and its correction clearly, preferably as **wrong → correct**.
- Give one short explanation of why the correction is needed when useful.
- Then give the correct model and ask the learner to try the same target again.
- Do not move to the next target while the current target is still unresolved.
- Do not merely say "wrong" or silently replace the learner's answer.
- Correct only errors that matter for the current target; do not overload a beginner with unrelated corrections.
- Example pattern: **Er heißen ❌ → Er heißt ✅.** Then briefly explain and ask the learner to produce the sentence again.
- If the learner's answer is correct and natural but differs from the model sentence, accept it. Do not require exact wording.
...
- When `target_completed` is true and a next target exists, the learner-facing reply MUST NOT stop after praise or confirmation. It MUST immediately continue into the **NEXT TARGET** in the SAME response, but it must first briefly teach/model the next target before asking the learner to perform it.
- The completion response MUST contain: (1) brief success feedback in the selected explanation language, (2) a short model/teaching cue for the next target, and (3) ONE concrete prompt that makes the learner perform the next target.
- Never end a completed-target response with only "good", "correct", "excellent", or another acknowledgement.
- When `target_completed` is false, remain focused on the current target and ask for another attempt when appropriate.
...
- If incorrect or incomplete, identify the useful error, give the smallest useful correction/model, and ask for another attempt.
...
def _practice_system_prompt(*, lesson: CourseLesson, targets: list[dict], scenario: dict | None, is_start: bool) -> str:
    task = "**START**: Begin naturally from the scenario, use one suitable target, and invite one response." if is_start else "**CONTINUE**: Respond to the learner's actual message and use it to continue naturally."
    return f"""
...
def _system_prompt(*, stage: str, lesson: CourseLesson, targets: list[dict], scenario: dict | None, current_target_order: int | None, is_start: bool) -> str:
    if stage == "teaching":
        return _teaching_system_prompt(lesson=lesson, targets=targets, current_target_order=current_target_order, is_start=is_start)
...
def _extract_target_completion(reply: str) -> tuple[str, int | None, bool]:
    ...
    if target_match:
        try:
            ...
            target_order = None
    clean_reply = _remove_control_markers(reply)
    return clean_reply, target_order, teaching_complete
...
def _parse_teaching_evaluation(raw_reply: str, *, current_target_order: int | None, is_control_message: bool) -> tuple[str, bool, int | None, bool]:
    try:
        ...
        valid_marker = not is_control_message and marker_target is not None and current_target_order is not None and marker_target == current_target_order
    ...
    valid_target_completion = not is_control_message and target_completed and current_target_order is not None and target_order == current_target_order
...
def _stream_stage_response(*, request: StageChatRequest, user_id: int, profile_id: int, lesson_id: int, conversation_id: str):
    ...
    try:
        ...
        set_explanation_language_context(mode=explanation_mode, native_language=user.native_language, learning_language=user.learning_language)
        ...
        context_message_limit = MAX_TEACHING_CONTEXT_MESSAGES if request.stage == "teaching" else MAX_PRACTICE_CONTEXT_MESSAGES
        messages = _history_messages(history, context_message_limit)
        ...
        response = provider.generate_text(model=AI_MODEL, prompt=messages, system_instruction=_system_prompt(stage=request.stage, lesson=lesson, targets=targets, scenario=scenario, current_target_order=current_target_order, is_start=is_control_message), max_output_tokens=MAX_OUTPUT_TOKENS, response_mime_type="application/json" if request.stage == "teaching" else None)
        raw_reply = str(response.text or "").strip()
        if not raw_reply:
            raise RuntimeError("AI tutor returned an empty response.")
        if request.stage == "teaching":
            reply, valid_target_completion, completed_target_order, teaching_complete_marker = _parse_teaching_evaluation(raw_reply, current_target_order=current_target_order, is_control_message=is_control_message)
        else:
        ...
        if request.stage == "teaching":
            ...
            if not stage_completed:
                required_orders = [int(target["order"]) for target in required_targets]
                for order in required_orders:
        ...
        if request.stage == "teaching":
            ...
            yield sse_event("decision", {"action": action, "target_id": completed_target_order if valid_target_completion else None, "next_target_id": next_target_order, "confidence": None})
        else:
            yield sse_event("decision", {"action": "CONTINUE", "target_id": None, "confidence": None})
        yield sse_event("done", {"conversation_id": conversation_id, "stage": request.stage, "axis_completed": stage_completed, "lesson_completed": request.stage == "practice" and stage_completed, "action": "AXIS_COMPLETE" if stage_completed else ("TARGET_COMPLETE" if valid_target_completion else "CONTINUE"), "target_id": completed_target_order if valid_target_completion else None, "next_target_id": next_target_order, "exchange_count": exchange_count_after})
...
def lesson_stage_chat(request: StageChatRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return StreamingResponse(_stream_stage_response(request=request, user_id=current_user.id, profile_id=profile.id, lesson_id=lesson.id, conversation_id=conversation_id), media_type="text/event-stream", headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"})
