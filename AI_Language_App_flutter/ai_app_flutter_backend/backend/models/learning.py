...
class UserLessonProgress(Base):
    ...
    # Compact lesson-practice state. This intentionally stores only the
    # current conversation id and the ids of target sentences the learner
    # has actually practiced; it does not store the lesson transcript.
...
class Word(Base):
    ...
    vocabulary_entry_id: Mapped[int | None] = mapped_column(
        ForeignKey("vocabulary_entries.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    vocabulary_form_id: Mapped[int | None] = mapped_column(
        ForeignKey("vocabulary_forms.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
